import { type Page } from "@playwright/test";
import { BasePage } from "./BasePage";

export class AdminWorkersPage extends BasePage {
  constructor(page: Page) {
    super(page);
  }

  async goto() {
    await this.page.goto("/admin/workers");
  }

  get heading() {
    return this.page.getByText(/Майстри та персонал/i).first();
  }

  get addButton() {
    return this.page.getByRole("button", { name: /Додати працівника/i });
  }

  get emailInput() {
    return this.page.locator('input[placeholder="email@example.com"]');
  }

  get usernameInput() {
    return this.page.locator('input[placeholder="username"]');
  }

  get firstNameInput() {
    return this.page.locator('input[placeholder="Іван"]');
  }

  get lastNameInput() {
    return this.page.locator('input[placeholder="Петренко"]');
  }

  get passwordInput() {
    return this.page.locator('input[placeholder="Мінімум 6 символів"]');
  }

  get occupationSelect() {
    return this.page.locator("select");
  }

  get saveButton() {
    return this.page.getByRole("button", { name: /Створити працівника/i });
  }

  get tableRows() {
    return this.page.locator("table tbody tr");
  }

  workerInList(name: string) {
    return this.page.getByText(name, { exact: false });
  }

  async openCreateForm() {
    await this.addButton.click();
    await this.page.waitForTimeout(300);
  }

  async fillCreateForm(data: {
    email: string;
    username: string;
    firstName: string;
    lastName: string;
    password: string;
    occupation?: string;
  }) {
    await this.emailInput.fill(data.email);
    await this.usernameInput.fill(data.username);
    await this.firstNameInput.fill(data.firstName);
    await this.lastNameInput.fill(data.lastName);
    await this.passwordInput.fill(data.password);
    if (data.occupation) {
      await this.occupationSelect.selectOption(data.occupation);
    }
  }
}
