    ...
    async ({ browser }) => {
      ...
      if (assigneeVisible) {
        ...
        if (guardOptionVisible) {
          ...
          await adminPage.waitForTimeout(500); // wait for Shadcn Select to register value
          await adminIssues.dialogSaveButton.click();
          // waitFor is more reliable than isVisible for detecting transient toast elements
      ...
      // Wait for tab panel to render (the component may show a spinner while loading)
      // then verify assigned issue content appears
      await guardPageObj.waitForTimeout(500);
      const assignedContent = guardPageObj.getByText(/Всього|No Issue|assigned/i).first();
